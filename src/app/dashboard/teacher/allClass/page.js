        ...
        {clases?.map((cls) => (
          <div key={cls?._id} className="card bg-base-100 shadow-xl">
            <figure className="overflow-hidden">
              <img
                className="w-full h-[250px] transition-all duration-300 hover:scale-110"
                src={cls?.image}
                alt="classes"
              />
            </figure>
            <div className="card-body">
              ...
              <div className="flex bg-slate-300 p-2 rounded-lg items-center justify-between">
                <button className="bg-black text-xl font-medium text-white px-2  rounded-md">
                  <Link
                    href={`/dashboard/teacher/allClass/SeeDetails/${cls?._id}`}
                  >
                    See Details
                  </Link>
                </button>
                <div className="flex gap-2 hover:cursor-pointer">
                  <Link
                    href={`/dashboard/teacher/allClass/editClass/${cls?._id}`}
                  >
